...
namespace TigerForge
{
    ...
    public class EventManager {
        ...
        /// </summary>
        public static void StartListening(string eventName, UnityAction callBack, bool saveCallBack = false)
        {
            ...
            if (saveCallBack) callBacks.Add(eventName, callBack);
        }
        ...
        /// </summary>
        public static void StartListening(string eventName, GameObject target, UnityAction callBack, bool saveCallBack = false)
        {
            if (target == null)
            {
            ...
            if (saveCallBack) callBacks.Add(eventName + "_EXTRA", callBack);
        ...
        /// <summary>
        /// Stop listening to the event with the given name. The callBack function must be saved by setting 'saveCallBack' parameter to true in the StartListening method.
        /// </summary>
        public static void StopListening(string eventName)
        {
            if (eventDictionary.TryGetValue(eventName, out UnityEvent thisEvent))
            {
                if (callBacks.ContainsKey(eventName))
                {
                    ...
                    if (eventDictionary.ContainsKey(eventName + "_EXTRA")) eventDictionary.Remove(eventName + "_EXTRA");
    ...
    public class EventsGroup
    {
        ...
        public void StartListening()
        {
            ...
            {
                EventManager.StartListening(g.name, g.callBack, true);
            }
        }
        ...
        public void StopListening(string eventName = "")
        {
            if (eventName == "")
            {
                foreach (SEvent g in group)
                {
                    EventManager.StopListening(g.name);
                }
            }
            else
            {
                ...
                foreach (SEvent g in group)
                {
                    if (g.name != eventName) newGroup.Add(g); else EventManager.StopListening(g.name);
                }
                group = newGroup;
            }
